...
import python.eapi.methods.loanprograms.loanprogram_helpers as lphelp


class LoanProgram(BaseRequest):
    """Class for Loan Program endpoint methods."""
    ...
    def get_multi(self, number=1, size=10):
        ...
        url = (
            vari.BASE_URL
            + "/v1/loan-programs?page[number]={}&page[size]={}".format(number,
                                                                       size)
        )
        response = self.get_endpoint(url)
        return json.loads(response.text)

    def get_one(self, lprog, expected_code=200):
        """
        Get one.

        :param lprog: json, an existing.
        :param expected_code: int, the expected response code.
        :return: json, the response.
        """

        url = vari.BASE_URL + "/v1/loan-programs/{}".format(lprog["id"])
    ...
    def post_one(self):
        ...
        url = vari.BASE_URL + "/v1/loan-programs"
